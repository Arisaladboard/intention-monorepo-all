...
export const sendBucketsToBackend = async (bucketsData) => {
    try {
        const response = await fetch(`${backendAddress}/api/Bucket`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(bucketsData)
        });
        ...
        throw new Error(`Error sending buckets to backend: ${error}`);
    }
}

export const receiveBucketsFromBackend = async () => {
    try {
        const response = await fetch(`${backendAddress}/api/Bucket`, {
            method: 'GET',
        });
        ...
        const bucketsReceived = await response.json();
        console.log(`New buckets received: ${JSON.stringify(bucketsReceived)}`);
        return bucketsReceived;
    } catch (error) {
        throw new Error(`Error receiving buckets from backend: ${error}`);
    }
}
